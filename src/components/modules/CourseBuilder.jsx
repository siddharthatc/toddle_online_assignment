...
const CourseBuilder = () => {
  ...
  // Expose header height as CSS variable for sticky alignment
  useEffect(() => {
  ...
  const handleAddClick = type => {
    if (type === 'module') { setCurrentModule(null); setIsModuleModalOpen(true); }
    if (type === 'link') { setCurrentModuleId(null); setIsLinkModalOpen(true); }
    if (type === 'upload') { setCurrentModuleId(null); setIsUploadModalOpen(true); }
  };

  const handleSaveModule = module => {
    setModules(prev => currentModule ? prev.map(m => (m.id === module.id ? module : m)) : [...prev, module]);
    setIsModuleModalOpen(false);
    setCurrentModule(null);
  };

  const handleEditModule = module => { setCurrentModule(module); setIsModuleModalOpen(true); };
  ...
  const handleSaveLink = item => { setItems(prev => [...prev, item]); setIsLinkModalOpen(false); };
  const handleSaveUpload = item => { setItems(prev => [...prev, item]); setIsUploadModalOpen(false); };
  const handleDeleteItem = id => { setItems(prev => prev.filter(i => i.id !== id)); };

  const moveItemToModule = (itemId, moduleId) => {
    setItems(prev => prev.map(i => i.id === itemId ? { ...i, moduleId } : i));
  };

  const [, dropUnassigned] = useDrop({
    accept: 'ITEM',
    drop: dragged => moveItemToModule(dragged.id, null),
  });
  ...
  const filteredModules = useMemo(() => {
    ...
    return modules.filter(module => {
      ...
      return items.some(item => item.moduleId === module.id && item.title.toLowerCase().includes(q));
    });
  ...
  const showOutline = modules.length >= 2;

  return (
    <div className="course-builder-container">
      {/* LEFT COLUMN: Header and Modules in the same container */}
      ...
      {/* RIGHT COLUMN: Independent Outline flexed horizontally */}
      {showOutline && (
              ...
              return (
                <div key={module.id}>
                  <button
                    ...
                    onClick={() => setExpandedOutlineModule(isExpanded ? null : module.id)}
                  >
                    {module.name}
                  </button>
                  {isExpanded && moduleItems.map(item => (
                    <button
                      ...
                      onClick={() => document.getElementById(`item-${item.id}`)?.scrollIntoView({ behavior: 'smooth' })}
                    >
                      {item.title}
                    </button>
                  ))}
      ...
      {/* MODALS */}
      <ModuleModal isOpen={isModuleModalOpen} onClose={() => setIsModuleModalOpen(false)} onSave={handleSaveModule} module={currentModule} />
      <LinkModal isOpen={isLinkModalOpen} onClose={() => setIsLinkModalOpen(false)} onSave={handleSaveLink} moduleId={currentModuleId} />
      <UploadModal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} onSave={handleSaveUpload} moduleId={currentModuleId} />
    </div>
  );
};
